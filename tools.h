...
void correctConditions(int n, condition *list, int *indices) {
  for (int i = 0; i < n; i++) indices[i] = list[i].getNode1();
...
void addExtension(char *newfilename, char *filename, string extension) {
  ...
  int i;
  for (i = 0; i < ori_length; i++) newfilename[i] = filename[i];
  for (i = 0; i < ext_length; i++) newfilename[ori_length + i] = extension[i];
...
void writeResults(mesh m, Vector T, char *filename) {
  ...
  file << "Result \"Temperature\" \"Load Case 1\" 1 Scalar "
          "OnNodes\nComponentNames \"T\"\nValues\n";
