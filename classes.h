...
class item {
 ...
 public:
  ...
  virtual void setValues(int a, float b, float c, int d, int e, int f,
                         float g) = 0;
};
...
class mesh {
 ...
 public:
  void setParameters(float E_, float A_, float f_) {
    parameters[E] = E_;
    parameters[A] = A_;
    parameters[f] = f_;
  }
  ...
  int* getDirichletIndices() { return indices_dirich; }
  condition* getDirichlet() { return dirichlet_list; }
  condition* getNeumann() { return neumann_list; }
