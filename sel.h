...
Matrix createLocalK(int element, mesh &m) {
  float D, Ae, E_ = m.getParameter(E), A_ = m.getParameter(A);
  Matrix K, A, B, Bt, At;
...
void crearSistemasLocales(mesh &m, vector<Matrix> &localKs,
